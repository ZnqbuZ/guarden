...
///   * `export(wrapped)`: Wrap all captured variables in a local struct, which can be accessed via `Deref`/`DerefMut` of the guard. Nothing is exported.
...
///   > the original bindings. This changes their type and ownership semantics. If you need to preserve 
...
mod tests {
    ...
    fn drop_sync_triggers_guard() {
        ...
        {
            crate::guarded!([called = called.clone()] {
                called.fetch_add(1, Ordering::SeqCst);
            });
        }
    ...
    fn guarded_named_mut_binding_updates_context_before_drop() {
...

        {
            let value = 1usize;
            let step = 2usize;
            crate::guarded!(scope_guard => [mut value, step, committed = committed.clone()] {
    ...
    #[test]
    fn init_capture_bindings_do_not_leak_or_shadow_outer_scope() {
        let committed = Arc::new(AtomicUsize::new(0));

        {
            let mut value = String::from("outside");
            let suffix = String::from(" suffix");

            crate::guarded!(scope_guard => [
                mut value = String::from("hello"),
                suffix = String::from(" world"),
                committed = committed.clone()
            ] {
                value.push_str(&suffix);
                committed.store(value.len(), Ordering::SeqCst);
            });

            value.push('!');
            assert_eq!(value.as_str(), "outside!");
            assert_eq!(suffix.as_str(), " suffix");

            drop(scope_guard);
        }

        assert_eq!(committed.load(Ordering::SeqCst), 11);
    }
    ...
    #[test]
    fn guarded_mut_guard_with_export_wrapped() {
        let committed = Arc::new(AtomicUsize::new(0));

        {
            crate::guarded!(mut scope_guard => export(wrapped) [
                value = 1usize,
                step = 2usize,
                committed = committed.clone()
            ] {
            ...
            scope_guard.value += 10;
            assert_eq!(scope_guard.value, 11);
            assert_eq!(scope_guard.step, 2);
...
mod tokio_tests {
    ...
    async fn drop_async_detaches_task() {
        ...
        {
            crate::guarded!([mut tx = Some(tx), value = 9usize] {
                let tx = tx.take();
                async move {
                    if let Some(tx) = tx {
                        let _ = tx.send(value);
